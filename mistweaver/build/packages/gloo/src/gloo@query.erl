...
-opaque 'query'(JZW) :: {'query',
        binary(),
        op(),
        gleam@dynamic@decode:decoder(JZW)}.
...
-spec from(gloo@schema:table(JZX)) -> 'query'(JZX).
from(Table) ->
    {'query',
...
-spec update('query'(KAF), list({binary(), gloo@value:gloo_value()})) -> 'query'(KAF).
update(Query, Sets) ->
    {'query',
...
-spec order_by('query'(KAP), binary(), direction()) -> 'query'(KAP).
...
-spec limit('query'(KAS), integer()) -> 'query'(KAS).
limit(Query, N) ->
    case erlang:element(3, Query) of
...
-spec offset('query'(KAV), integer()) -> 'query'(KAV).
offset(Query, N) ->
    case erlang:element(3, Query) of
...
-spec returning('query'(any()), gleam@dynamic@decode:decoder(KBA)) -> 'query'(KBA).
returning(Query, Decoder) ->
    {'query', erlang:element(2, Query), erlang:element(3, Query), Decoder}.

-file("src/gloo/query.gleam", 175).
-spec returning_columns('query'(KBD), list(binary())) -> 'query'(KBD).
...
-spec decoder('query'(KBK)) -> gleam@dynamic@decode:decoder(KBK).
decoder(Query) ->
    erlang:element(4, Query).
