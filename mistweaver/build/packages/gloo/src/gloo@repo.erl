...
-spec all(
    ...
    gleam@dynamic@decode:decoder(KLB)
) -> {ok, list(KLB)} | {error, gloo@error:gloo_error()}.
...
-spec one_from_rows(list(KMB)) -> {ok, KMB} | {error, gloo@error:gloo_error()}.
one_from_rows(Rows) ->
    case Rows of
        [Row] ->
            {ok, Row};
...
-spec one(
    ...
    binary(),
    list(gloo@value:gloo_value()),
    gleam@dynamic@decode:decoder(KLH)
) -> {ok, KLH} | {error, gloo@error:gloo_error()}.
...
-spec maybe_one_from_rows(list(KMF)) -> {ok, gleam@option:option(KMF)} |
...
-spec maybe_one(
    ...
    binary(),
    list(gloo@value:gloo_value()),
    gleam@dynamic@decode:decoder(KLM)
) -> {ok, gleam@option:option(KLM)} | {error, gloo@error:gloo_error()}.
...
-spec transaction(
    repo(),
    fun((repo()) -> {ok, KLU} | {error, gloo@error:gloo_error()})
) -> {ok, KLU} | {error, gloo@error:gloo_error()}.
...
-spec query_all(repo(), gloo@query:'query'(KMK)) -> {ok, list(KMK)} |
    {error, gloo@error:gloo_error()}.
query_all(Repo, Q) ->
...
-spec query_one(repo(), gloo@query:'query'(KMP)) -> {ok, KMP} |
    {error, gloo@error:gloo_error()}.
query_one(Repo, Q) ->
    {Sql, Params} = gloo@query:to_sql(Q),
...
-spec query_maybe_one(repo(), gloo@query:'query'(KMT)) -> {ok,
        gleam@option:option(KMT)} |
    {error, gloo@error:gloo_error()}.
...
-spec sql_all(repo(), gloo@sql:sql(KNC)) -> {ok, list(KNC)} |
    {error, gloo@error:gloo_error()}.
sql_all(Repo, S) ->
...
-spec sql_one(repo(), gloo@sql:sql(KNH)) -> {ok, KNH} |
    {error, gloo@error:gloo_error()}.
sql_one(Repo, S) ->
    {Statement, Parameters, Decoder} = gloo@sql:to_parts(S),
...
-spec sql_maybe_one(repo(), gloo@sql:sql(KNL)) -> {ok, gleam@option:option(KNL)} |
    {error, gloo@error:gloo_error()}.
