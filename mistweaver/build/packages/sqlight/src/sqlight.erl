...
-spec with_connection(binary(), fun((connection()) -> DLB)) -> DLB.
...
-spec nullable(fun((DLR) -> value()), gleam@option:option(DLR)) -> value().
...
-spec 'query'(
    ...
    connection(),
    list(value()),
    gleam@dynamic@decode:decoder(DLF)
) -> {ok, list(DLF)} | {error, error()}.
