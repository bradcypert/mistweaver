...
-spec be_ok({ok, JRS} | {error, any()}) -> JRS.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, JRX}) -> JRX.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(JSA)) -> JSA.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
