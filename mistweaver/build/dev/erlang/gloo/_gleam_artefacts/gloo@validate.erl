...
-spec struct(list({ok, OPI} | {error, list(error())})) -> {ok, list(OPI)} |
    {error, list(error())}.
struct(Fields) ->
...
-spec field(binary(), OPR, list(fun((OPR) -> {ok, nil} | {error, binary()}))) -> {ok,
        OPR} |
    {error, list(error())}.
...
-spec required(binary(), gleam@option:option(OPY)) -> {ok, OPY} |
    {error, list(error())}.
