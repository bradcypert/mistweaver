...
-opaque sql(OBR) :: {sql,
        ...
        gleam@dynamic@decode:decoder(OBR),
        integer()}.
...
-spec nullable(fun((OBS) -> gloo@value:gloo_value()), gleam@option:option(OBS)) -> gloo@value:gloo_value().
nullable(Encoder, V) ->
...
-spec param(sql(OBV), gloo@value:gloo_value()) -> sql(OBV).
param(Sql, V) ->
    {sql,
...
-spec params(sql(OBY), list(gloo@value:gloo_value())) -> sql(OBY).
params(Sql, Values) ->
    gleam@list:fold(Values, Sql, fun param/2).

-file("src/gloo/sql.gleam", 86).
-spec returns(sql(any()), gleam@dynamic@decode:decoder(OCE)) -> sql(OCE).
returns(Sql, Decoder) ->
    {sql,
        erlang:element(2, Sql),
...
-spec in_clause(sql(OCH), list(gloo@value:gloo_value())) -> {sql(OCH), binary()}.
...
-spec unnest(fun((OCL) -> gloo@value:gloo_value()), list(OCL)) -> gloo@value:gloo_value().
unnest(Encoder, Values) ->
    {g_array, gleam@list:map(Values, Encoder)}.

-file("src/gloo/sql.gleam", 116).
-spec to_parts(sql(OCN)) -> {binary(),
    list(gloo@value:gloo_value()),
    gleam@dynamic@decode:decoder(OCN)}.
to_parts(Sql) ->
