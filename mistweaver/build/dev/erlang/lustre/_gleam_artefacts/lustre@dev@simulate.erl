...
-opaque app(ITM, ITN, ITO) :: {app,
        fun((ITM) -> {ITN, lustre@effect:effect(ITO)}),
        fun((ITN, ITO) -> {ITN, lustre@effect:effect(ITO)}),
        fun((ITN) -> lustre@vdom@vnode:element(ITO))}.

-opaque simulation(ITP, ITQ) :: {simulation,
        fun((ITP, ITQ) -> {ITP, lustre@effect:effect(ITQ)}),
        fun((ITP) -> lustre@vdom@vnode:element(ITQ)),
        list(event(ITQ)),
        ITP,
        lustre@vdom@vnode:element(ITQ)}.

-type event(ITR) :: {dispatch, ITR} |
...
-spec simple(
    fun((ITS) -> ITT),
    fun((ITT, ITU) -> ITT),
    fun((ITT) -> lustre@vdom@vnode:element(ITU))
) -> app(ITS, ITT, ITU).
...
-spec application(
    fun((ITZ) -> {IUA, lustre@effect:effect(IUB)}),
    fun((IUA, IUB) -> {IUA, lustre@effect:effect(IUB)}),
    fun((IUA) -> lustre@vdom@vnode:element(IUB))
) -> app(ITZ, IUA, IUB).
...
-spec start(app(IUI, IUJ, IUK), IUI) -> simulation(IUJ, IUK).
start(App, Args) ->
...
-spec message(simulation(IUQ, IUR), IUR) -> simulation(IUQ, IUR).
...
-spec problem(simulation(IVW, IVX), binary(), binary()) -> simulation(IVW, IVX).
...
-spec click(simulation(IVD, IVE), lustre@dev@query:'query'()) -> simulation(IVD, IVE).
...
-spec input(simulation(IVJ, IVK), lustre@dev@query:'query'(), binary()) -> simulation(IVJ, IVK).
...
-spec model(simulation(IWC, any())) -> IWC.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), IWH)) -> lustre@vdom@vnode:element(IWH).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), IWM)) -> list(event(IWM)).
history(Simulation) ->
