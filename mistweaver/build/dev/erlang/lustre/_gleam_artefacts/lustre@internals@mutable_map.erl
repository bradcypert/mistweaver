...
-type mutable_map(CFF, CFG) :: any() | {gleam_phantom, CFF, CFG}.
...
?DOC(false).
-spec unsafe_get(mutable_map(CFL, CFM), CFL) -> CFM.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(CFT, CFU), CFT, fun(() -> CFU)) -> CFU.
...
?DOC(false).
-spec has_key(mutable_map(CGD, any()), CGD) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(CGL, CGM), CGL, CGM) -> mutable_map(CGL, CGM).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(CGX, CGY), CGX) -> mutable_map(CGX, CGY).
delete(Map, Key) ->
    maps:remove(Key, Map).
