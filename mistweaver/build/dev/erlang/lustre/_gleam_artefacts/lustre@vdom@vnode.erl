...
-type element(CIN) :: {fragment,
        integer(),
        binary(),
        list(element(CIN)),
        lustre@internals@mutable_map:mutable_map(binary(), element(CIN))} |
    {element,
        integer(),
        binary(),
        binary(),
        binary(),
        list(lustre@vdom@vattr:attribute(CIN)),
        list(element(CIN)),
        lustre@internals@mutable_map:mutable_map(binary(), element(CIN)),
        boolean(),
        boolean()} |
...
-spec fragment(
    binary(),
    list(element(CIV)),
    lustre@internals@mutable_map:mutable_map(binary(), element(CIV))
) -> element(CIV).
...
-spec element(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(CJC)),
    list(element(CJC)),
    lustre@internals@mutable_map:mutable_map(binary(), element(CJC)),
    boolean(),
    boolean()
) -> element(CJC).
element(
...
-spec unsafe_inner_html(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(CJN)),
    binary()
) -> element(CJN).
...
?DOC(false).
-spec map(element(CJR), fun((CJR) -> CJT)) -> element(CJT).
map(Element, Mapper) ->
    case Element of
...
?DOC(false).
-spec memo(binary(), list(lustre@internals@ref:ref()), fun(() -> element(CJW))) -> element(CJW).
memo(Key, Dependencies, View) ->
    {memo, 5, Key, Dependencies, View}.

-file("src/lustre/vdom/vnode.gleam", 201).
?DOC(false).
-spec to_keyed(binary(), element(CKB)) -> element(CKB).
to_keyed(Key, Node) ->
    case Node of
...
-spec memo_to_json(
    fun(() -> element(CKE)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(CKE)), element(CKE))
) -> gleam@json:json().
...
-spec element_to_json(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(CKE)),
    list(element(CKE)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(CKE)), element(CKE))
) -> gleam@json:json().
