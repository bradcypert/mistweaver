...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, HFW} | {error, nil})) -> lustre@runtime@app:option(HFW).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(HGA)) -> lustre@runtime@app:option(HGA).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(HGD)) -> lustre@runtime@app:option(HGD).
...
-spec on_form_autofill(fun((binary()) -> HGI)) -> lustre@runtime@app:option(HGI).
...
-spec on_form_reset(HGK) -> lustre@runtime@app:option(HGK).
...
-spec on_form_restore(fun((binary()) -> HGM)) -> lustre@runtime@app:option(HGM).
...
-spec on_form_disabled(fun((boolean()) -> HGO)) -> lustre@runtime@app:option(HGO).
...
-spec on_connect(HGW) -> lustre@runtime@app:option(HGW).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(HGY) -> lustre@runtime@app:option(HGY).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(HHA) -> lustre@runtime@app:option(HHA).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(HHC)),
    list(lustre@vdom@vnode:element(HHC))
) -> lustre@vdom@vnode:element(HHC).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(HHI)),
    list(lustre@vdom@vnode:element(HHI))
) -> lustre@vdom@vnode:element(HHI).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), HII),
    binary(),
    list(lustre@vdom@vattr:attribute(HII)),
    list(lustre@vdom@vnode:element(HII))
) -> lustre@vdom@vnode:element(HII).
