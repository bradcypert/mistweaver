...
-type attribute(AYI) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(AYI)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(AYJ) :: {handler, boolean(), boolean(), AYJ}.
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(AYO)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
...
?DOC(false).
-spec merge(list(attribute(AYY)), list(attribute(AYY))) -> list(attribute(AYY)).
...
?DOC(false).
-spec compare(attribute(AZF), attribute(AZF)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(AYT))) -> list(attribute(AYT)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
