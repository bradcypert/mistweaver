...
-type runtime(HUZ) :: any() | {gleam_phantom, HUZ}.
...
-spec application(
    fun((HVV) -> {HVW, lustre@effect:effect(HVX)}),
    fun((HVW, HVX) -> {HVW, lustre@effect:effect(HVX)}),
    fun((HVW) -> lustre@vdom@vnode:element(HVX))
) -> lustre@runtime@app:app(HVV, HVW, HVX).
...
-spec element(lustre@vdom@vnode:element(HVI)) -> lustre@runtime@app:app(any(), nil, HVI).
element(View) ->
    application(
...
-spec simple(
    fun((HVO) -> HVP),
    fun((HVP, HVQ) -> HVP),
    fun((HVP) -> lustre@vdom@vnode:element(HVQ))
) -> lustre@runtime@app:app(HVO, HVP, HVQ).
...
-spec component(
    fun((HWE) -> {HWF, lustre@effect:effect(HWG)}),
    fun((HWF, HWG) -> {HWF, lustre@effect:effect(HWG)}),
    fun((HWF) -> lustre@vdom@vnode:element(HWG)),
    list(lustre@runtime@app:option(HWG))
) -> lustre@runtime@app:app(HWE, HWF, HWG).
...
-spec named(
    lustre@runtime@app:app(HWP, HWQ, HWR),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(HWR))
) -> lustre@runtime@app:app(HWP, HWQ, HWR).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(HXJ, any(), HXL), binary(), HXJ) -> {ok,
        runtime(HXL)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec start(lustre@runtime@app:app(HXA, any(), HXC), binary(), HXA) -> {ok,
        runtime(HXC)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(HXS, any(), HXU), HXS) -> {ok,
        runtime(HXU)} |
    {error, error()}.
...
-spec supervised(lustre@runtime@app:app(HYB, any(), HYD), HYB) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(HYD))).
...
-spec factory(lustre@runtime@app:app(HYK, any(), HYM)) -> gleam@otp@factory_supervisor:builder(HYK, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(HYM))).
factory(App) ->
...
-spec send(runtime(HZB), lustre@runtime@server@runtime:message(HZB)) -> nil.
...
-spec dispatch(HZE) -> lustre@runtime@server@runtime:message(HZE).
dispatch(Message) ->
