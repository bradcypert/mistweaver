...
-type diff(FEQ) :: {diff,
        lustre@vdom@patch:patch(FEQ),
        lustre@vdom@cache:cache(FEQ)}.

-type partial_diff(FER) :: {partial_diff,
        lustre@vdom@patch:patch(FER),
        lustre@vdom@cache:cache(FER),
        lustre@vdom@cache:events(FER)}.

-type attribute_change(FES) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(FES)),
        list(lustre@vdom@vattr:attribute(FES)),
        lustre@vdom@cache:events(FES)}.
...
-spec diff_attributes(
    ...
    lustre@vdom@cache:events(FFV),
    list(lustre@vdom@vattr:attribute(FFV)),
    list(lustre@vdom@vattr:attribute(FFV)),
    list(lustre@vdom@vattr:attribute(FFV)),
    list(lustre@vdom@vattr:attribute(FFV))
) -> attribute_change(FFV).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(FEY)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(FEY)),
    list(lustre@vdom@vnode:element(FEY)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(FEY)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(FEY)),
    list(lustre@vdom@patch:patch(FEY)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(FEY),
    lustre@vdom@cache:events(FEY)
) -> partial_diff(FEY).
do_diff(
    Old,
    Old_keyed,
...
?DOC(false).
-spec diff(
    lustre@vdom@cache:cache(FET),
    lustre@vdom@vnode:element(FET),
    lustre@vdom@vnode:element(FET)
) -> diff(FET).
diff(Cache, Old, New) ->
