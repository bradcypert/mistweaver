...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(JEN)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(JEN)),
    list(lustre@vdom@vnode:element(JEN))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(JEN)),
    list(lustre@vdom@vnode:element(JEN))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(JEF)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(JEF)),
    list(lustre@vdom@vnode:element(JEF))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(JCL)),
    list({binary(), lustre@vdom@vnode:element(JCL)})
) -> lustre@vdom@vnode:element(JCL).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(JCR)),
    list({binary(), lustre@vdom@vnode:element(JCR)})
) -> lustre@vdom@vnode:element(JCR).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(JCX)})) -> lustre@vdom@vnode:element(JCX).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(JDB)),
    list({binary(), lustre@vdom@vnode:element(JDB)})
) -> lustre@vdom@vnode:element(JDB).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(JDH)),
    list({binary(), lustre@vdom@vnode:element(JDH)})
) -> lustre@vdom@vnode:element(JDH).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(JDN)),
    list({binary(), lustre@vdom@vnode:element(JDN)})
) -> lustre@vdom@vnode:element(JDN).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(JDT)),
    list({binary(), lustre@vdom@vnode:element(JDT)})
) -> lustre@vdom@vnode:element(JDT).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(JDZ)),
    list({binary(), lustre@vdom@vnode:element(JDZ)})
) -> lustre@vdom@vnode:element(JDZ).
