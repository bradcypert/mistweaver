...
-type change(ECN) :: {replace_text, integer(), binary()} |
    {replace_inner_html, integer(), binary()} |
    {update,
        integer(),
        list(lustre@vdom@vattr:attribute(ECN)),
        list(lustre@vdom@vattr:attribute(ECN))} |
    {move, integer(), binary(), integer()} |
    {replace, integer(), integer(), lustre@vdom@vnode:element(ECN)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(ECN)), integer()}.

-file("src/lustre/vdom/patch.gleam", 77).
?DOC(false).
-spec new(integer(), integer(), list(change(ECO)), list(patch(ECO))) -> patch(ECO).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(ECY)),
    list(lustre@vdom@vattr:attribute(ECY))
) -> change(ECY).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(EDI)) -> change(EDI).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 127).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(EDL)), integer()) -> change(EDL).
...
?DOC(false).
-spec add_parent(patch(EDR), integer()) -> patch(EDR).
...
-spec insert_to_json(
    integer(),
    list(lustre@vdom@vnode:element(EGI)),
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(EGI)), lustre@vdom@vnode:element(EGI))
) -> gleam@json:json().
...
-spec replace_to_json(
    integer(),
    integer(),
    lustre@vdom@vnode:element(EGK),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(EGK)), lustre@vdom@vnode:element(EGK))
) -> gleam@json:json().
...
-spec change_to_json(
    change(EDX),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(EDX)), lustre@vdom@vnode:element(EDX))
) -> gleam@json:json().
...
-spec to_json(
    patch(EDU),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(EDU)), lustre@vdom@vnode:element(EDU))
) -> gleam@json:json().
to_json(Patch, Memos) ->
