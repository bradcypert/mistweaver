...
-opaque effect(BXT) :: {effect,
        list(fun((actions(BXT)) -> nil)),
        list(fun((actions(BXT)) -> nil)),
        list(fun((actions(BXT)) -> nil))}.

-type actions(BXU) :: {actions,
        fun((BXU) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(BXU)) -> nil),
        fun(() -> gleam@dynamic:dynamic_()),
        fun((binary(), gleam@json:json()) -> nil),
        fun((binary(), gleam@dynamic@decode:decoder(BXU)) -> nil),
        fun((binary()) -> nil)}.
...
-spec from(fun((fun((BXX) -> nil)) -> nil)) -> effect(BXX).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((BXZ) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(BXZ).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((BYB) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(BYB).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((BYF) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(BYF))
) -> effect(BYF).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec subscribe(binary(), gleam@dynamic@decode:decoder(BYM)) -> effect(BYM).
...
-spec batch(list(effect(BYR))) -> effect(BYR).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
...
-spec do_comap_select(
    actions(BZJ),
    gleam@erlang@process:selector(BZL),
    fun((BZL) -> BZJ)
) -> nil.
...
-spec do_comap_actions(actions(BZF), fun((BZH) -> BZF)) -> actions(BZH).
...
-spec do_map(list(fun((actions(BYZ)) -> nil)), fun((BYZ) -> BZC)) -> list(fun((actions(BZC)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(BYV), fun((BYV) -> BYX)) -> effect(BYX).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((BZN) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(BZN)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil),
    fun((binary(), gleam@dynamic@decode:decoder(BZN)) -> nil),
    fun((binary()) -> nil)
) -> nil.
