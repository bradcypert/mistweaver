...
-spec on(binary(), gleam@dynamic@decode:decoder(KDM)) -> lustre@vdom@vattr:attribute(KDM).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(KDP))
) -> lustre@vdom@vattr:attribute(KDP).
...
-spec handler(KDT, boolean(), boolean()) -> lustre@vdom@vattr:handler(KDT).
...
-spec prevent_default(lustre@vdom@vattr:attribute(KDV)) -> lustre@vdom@vattr:attribute(KDV).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(KDY)) -> lustre@vdom@vattr:attribute(KDY).
...
-spec debounce(lustre@vdom@vattr:attribute(KEB), integer()) -> lustre@vdom@vattr:attribute(KEB).
...
-spec throttle(lustre@vdom@vattr:attribute(KEE), integer()) -> lustre@vdom@vattr:attribute(KEE).
...
-spec on_keypress(fun((binary()) -> KEV)) -> lustre@vdom@vattr:attribute(KEV).
on_keypress(Message) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> KEX)) -> lustre@vdom@vattr:attribute(KEX).
on_keydown(Message) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> KEZ)) -> lustre@vdom@vattr:attribute(KEZ).
on_keyup(Message) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> KFB)) -> lustre@vdom@vattr:attribute(KFB).
on_input(Message) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> KFD)) -> lustre@vdom@vattr:attribute(KFD).
on_change(Message) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> KFF)) -> lustre@vdom@vattr:attribute(KFF).
on_check(Message) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> KFI)) -> lustre@vdom@vattr:attribute(KFI).
on_submit(Message) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(KFM) -> lustre@vdom@vattr:attribute(KFM).
on_focus(Message) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Message)).

-file("src/lustre/event.gleam", 357).
-spec on_blur(KFO) -> lustre@vdom@vattr:attribute(KFO).
on_blur(Message) ->
