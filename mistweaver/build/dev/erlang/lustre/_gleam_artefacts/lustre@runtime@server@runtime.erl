...
-type state(FQK, FQL) :: {state,
        gleam@erlang@process:subject(message(FQL)),
        gleam@erlang@process:selector(message(FQL)),
        gleam@erlang@process:selector(message(FQL)),
        FQK,
        fun((FQK, FQL) -> {FQK, lustre@effect:effect(FQL)}),
        fun((FQK) -> lustre@vdom@vnode:element(FQL)),
        config(FQL),
        lustre@vdom@vnode:element(FQL),
        lustre@vdom@cache:cache(FQL),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(FQL)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(FQL)) -> nil))}.

-type config(FQM) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, FQM} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(FQM)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(FQM)),
        gleam@option:option(FQM),
        gleam@option:option(FQM)}.

-type message(FQN) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(FQN))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(FQN))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(FQN)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(FQN)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(FQN))} |
    {effect_dispatched_message, FQN} |
    ...
    {effect_requested_context_subscription,
        binary(),
        gleam@dynamic@decode:decoder(FQN)} |
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(FSN)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(FSN)) -> nil)),
    lustre@runtime@transport:client_message(FSN)
) -> nil.
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(FSJ)),
    lustre@effect:effect(FSJ)
) -> nil.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(FSD)),
    binary(),
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, FRW} | {error, nil})),
    binary(),
    binary()
) -> {ok, FRW} | {error, nil}.
...
?DOC(false).
-spec loop(state(FRG, FRH), message(FRH)) -> gleam@otp@actor:next(state(FRG, FRH), message(FRH)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(FQR))),
    fun((FQV) -> {FQW, lustre@effect:effect(FQR)}),
    fun((FQW, FQR) -> {FQW, lustre@effect:effect(FQR)}),
    fun((FQW) -> lustre@vdom@vnode:element(FQR)),
    config(FQR),
    FQV
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(FQR)))} |
