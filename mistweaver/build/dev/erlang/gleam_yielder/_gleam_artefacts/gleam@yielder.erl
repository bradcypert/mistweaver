...
-type action(DXW) :: stop | {continue, DXW, fun(() -> action(DXW))}.

-opaque yielder(DXX) :: {yielder, fun(() -> action(DXX))}.

-type step(DXY, DXZ) :: {next, DXY, DXZ} | done.

-type chunk(DYA, DYB) :: {another_by,
        list(DYA),
        DYB,
        DYA,
        fun(() -> action(DYA))} |
...
-type sized_chunk(DYC) :: {another, list(DYC), fun(() -> action(DYC))} |
    {last, list(DYC)} |
    no_more.
...
-spec unfold_loop(DYK, fun((DYK) -> step(DYL, DYK))) -> fun(() -> action(DYL)).
...
-spec unfold(DYF, fun((DYF) -> step(DYG, DYF))) -> yielder(DYG).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> DYP)) -> yielder(DYP).
repeatedly(F) ->
...
-spec from_list(list(DYT)) -> yielder(DYT).
from_list(List) ->
...
-spec transform(yielder(DZD), DZF, fun((DZF, DZD) -> step(DZG, DZF))) -> yielder(DZG).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(DZN)), fun((DZP, DZN) -> DZP), DZP) -> DZP.
...
-spec fold(yielder(DZK), DZM, fun((DZM, DZK) -> DZM)) -> DZM.
...
-spec to_list(yielder(DZS)) -> list(DZS).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(DZV)) -> step(DZV, yielder(DZV)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(EAD)), integer()) -> fun(() -> action(EAD)).
...
-spec take(yielder(EAA), integer()) -> yielder(EAA).
...
-spec drop_loop(fun(() -> action(EAJ)), integer()) -> action(EAJ).
...
-spec map(yielder(EAM), fun((EAM) -> EAO)) -> yielder(EAO).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(EAU), yielder(EAW), fun((EAU, EAW) -> EAY)) -> yielder(EAY).
...
-spec append_loop(fun(() -> action(EBK)), fun(() -> action(EBK))) -> action(EBK).
...
-spec append(yielder(EBG), yielder(EBG)) -> yielder(EBG).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(EBS)))) -> action(EBS).
...
-spec flatten(yielder(yielder(EBO))) -> yielder(EBO).
flatten(Yielder) ->
...
-spec concat(list(yielder(EBW))) -> yielder(EBW).
concat(Yielders) ->
...
-spec flat_map(yielder(ECA), fun((ECA) -> yielder(ECC))) -> yielder(ECC).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(ECI)), fun((ECI) -> boolean())) -> action(ECI).
...
-spec filter(yielder(ECF), fun((ECF) -> boolean())) -> yielder(ECF).
...
-spec filter_map(yielder(ECL), fun((ECL) -> {ok, ECN} | {error, any()})) -> yielder(ECN).
...
-spec cycle(yielder(ECZ)) -> yielder(ECZ).
cycle(Yielder) ->
...
-spec find_loop(fun(() -> action(EDH)), fun((EDH) -> boolean())) -> {ok, EDH} |
    {error, nil}.
...
-spec find(yielder(EDD), fun((EDD) -> boolean())) -> {ok, EDD} | {error, nil}.
...
-spec find_map(yielder(EDL), fun((EDL) -> {ok, EDN} | {error, any()})) -> {ok,
        EDN} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(EEE)), integer()) -> fun(() -> action({EEE,
    integer()})).
...
-spec index(yielder(EEB)) -> yielder({EEB, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(EEH, fun((EEH) -> EEH)) -> yielder(EEH).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(EEM)), fun((EEM) -> boolean())) -> fun(() -> action(EEM)).
...
-spec take_while(yielder(EEJ), fun((EEJ) -> boolean())) -> yielder(EEJ).
...
-spec drop_while_loop(fun(() -> action(EES)), fun((EES) -> boolean())) -> action(EES).
...
-spec drop_while(yielder(EEP), fun((EEP) -> boolean())) -> yielder(EEP).
...
-spec scan_loop(fun(() -> action(EEZ)), fun((EFB, EEZ) -> EFB), EFB) -> fun(() -> action(EFB)).
...
-spec scan(yielder(EEV), EEX, fun((EEX, EEV) -> EEX)) -> yielder(EEX).
...
-spec zip_loop(fun(() -> action(EFI)), fun(() -> action(EFK))) -> fun(() -> action({EFI,
    EFK})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(EFD), yielder(EFF)) -> yielder({EFD, EFF}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(EFX)), fun((EFX) -> EFZ), EFZ, list(EFX)) -> chunk(EFX, EFZ).
...
-spec chunk_loop(fun(() -> action(EFS)), fun((EFS) -> EFU), EFU, EFS) -> action(list(EFS)).
...
-spec chunk(yielder(EFN), fun((EFN) -> any())) -> yielder(list(EFN)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(EGL)), integer(), list(EGL)) -> sized_chunk(EGL).
...
-spec sized_chunk_loop(fun(() -> action(EGH)), integer()) -> fun(() -> action(list(EGH))).
...
-spec sized_chunk(yielder(EGD), integer()) -> yielder(list(EGD)).
...
-spec intersperse_loop(fun(() -> action(EGS)), EGS) -> action(EGS).
...
-spec intersperse(yielder(EGP), EGP) -> yielder(EGP).
...
-spec any_loop(fun(() -> action(EGX)), fun((EGX) -> boolean())) -> boolean().
...
-spec any(yielder(EGV), fun((EGV) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(EHB)), fun((EHB) -> boolean())) -> boolean().
...
-spec all(yielder(EGZ), fun((EGZ) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(EHR) -> fun((gleam@option:option(list(EHR))) -> list(EHR)).
...
-spec group_updater(fun((EHJ) -> EHK)) -> fun((gleam@dict:dict(EHK, list(EHJ)), EHJ) -> gleam@dict:dict(EHK, list(EHJ))).
group_updater(F) ->
...
-spec group(yielder(EHD), fun((EHD) -> EHF)) -> gleam@dict:dict(EHF, list(EHD)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(EHV), fun((EHV, EHV) -> EHV)) -> {ok, EHV} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(EHZ)) -> {ok, EHZ} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(EIH) -> yielder(EIH).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(EIN)), fun(() -> action(EIN))) -> action(EIN).
...
-spec interleave(yielder(EIJ), yielder(EIJ)) -> yielder(EIJ).
...
-spec fold_until_loop(
    fun(() -> action(EIV)),
    fun((EIX, EIV) -> gleam@list:continue_or_stop(EIX)),
    EIX
) -> EIX.
...
-spec fold_until(
    yielder(EIR),
    EIT,
    fun((EIT, EIR) -> gleam@list:continue_or_stop(EIT))
) -> EIT.
...
-spec try_fold(yielder(EIZ), EJB, fun((EJB, EIZ) -> {ok, EJB} | {error, EJC})) -> {ok,
        EJB} |
    {error, EJC}.
...
-spec first(yielder(EJP)) -> {ok, EJP} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(EJT), integer()) -> {ok, EJT} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(EKB), fun((EKB) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(EKE, fun(() -> yielder(EKE))) -> yielder(EKE).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(EKH), EKH) -> yielder(EKH).
