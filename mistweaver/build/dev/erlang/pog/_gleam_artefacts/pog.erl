...
-export([named_connection/1, host/2, port/2, database/2, user/2, password/2, ssl/2, connection_parameter/3, pool_size/2, queue_target/2, queue_interval/2, idle_interval/2, trace/2, ip_version/2, rows_as_map/2, default_config/1, url_config/2, start/1, supervised/1, null/0, bool/1, int/1, float/1, text/1, bytea/1, array/2, timestamp/1, timestamp_decoder/0, calendar_date/1, calendar_time_of_day/1, transaction/2, nullable/2, 'query'/1, returning/2, parameter/2, timeout/2, execute/2, error_code_name/1, calendar_date_decoder/0, calendar_time_of_day_decoder/0, numeric_decoder/0]).
...
-type transaction_error(MBF) :: {transaction_query_error, query_error()} |
    {transaction_rolled_back, MBF}.

-type returned(MBG) :: {returned, integer(), list(MBG)}.
...
-opaque 'query'(MBH) :: {'query',
        binary(),
        list(value()),
        gleam@dynamic@decode:decoder(MBH),
        integer()}.
...
-spec array(fun((MBY) -> value()), list(MBY)) -> value().
...
-spec transaction_layer(
    single_connection(),
    fun((connection()) -> {ok, MCJ} | {error, MCK})
) -> {ok, MCJ} | {error, transaction_error(MCK)}.
...
-spec transaction(connection(), fun((connection()) -> {ok, MCC} | {error, MCD})) -> {ok,
        MCC} |
    {error, transaction_error(MCD)}.
...
-spec nullable(fun((MCT) -> value()), gleam@option:option(MCT)) -> value().
...
-spec returning('query'(any()), gleam@dynamic@decode:decoder(MDF)) -> 'query'(MDF).
...
-spec parameter('query'(MDI), value()) -> 'query'(MDI).
...
-spec timeout('query'(MDL), integer()) -> 'query'(MDL).
...
-spec execute('query'(MDO), connection()) -> {ok, returned(MDO)} |
    {error, query_error()}.
execute(Query, Pool) ->
