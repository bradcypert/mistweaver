...
-opaque subject(GQI) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(GQI)}.

-type name(GQJ) :: any() | {gleam_phantom, GQJ}.

-type do_not_leak() :: any().

-type selector(GQK) :: any() | {gleam_phantom, GQK}.
...
-spec 'receive'(subject(GRI), integer()) -> {ok, GRI} | {error, nil}.
...
-spec receive_forever(subject(GRQ)) -> GRQ.
...
-spec selector_receive(selector(GRU), integer()) -> {ok, GRU} | {error, nil}.
...
-spec selector_receive_forever(selector(GRY)) -> GRY.
...
-spec map_selector(selector(GSA), fun((GSA) -> GSC)) -> selector(GSC).
map_selector(A, B) ->
...
-spec merge_selector(selector(GSE), selector(GSE)) -> selector(GSE).
merge_selector(A, B) ->
...
-spec select_trapped_exits(selector(GSI), fun((exit_message()) -> GSI)) -> selector(GSI).
...
-spec select_map(selector(GSP), subject(GSR), fun((GSR) -> GSP)) -> selector(GSP).
...
-spec select(selector(GSL), subject(GSL)) -> selector(GSL).
...
-spec deselect(selector(GSU), subject(any())) -> selector(GSU).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> GSZ)
) -> selector(GSZ).
...
-spec select_other(selector(GTD), fun((gleam@dynamic:dynamic_()) -> GTD)) -> selector(GTD).
...
-spec select_specific_monitor(selector(GTP), monitor(), fun((down()) -> GTP)) -> selector(GTP).
...
-spec select_monitors(selector(GTS), fun((down()) -> GTS)) -> selector(GTS).
...
-spec deselect_specific_monitor(selector(GTV), monitor()) -> selector(GTV).
...
-spec perform_call(
    subject(GTY),
    fun((subject(GUA)) -> GTY),
    fun((selector(GUA)) -> {ok, GUA} | {error, nil})
) -> GUA.
...
-spec call(subject(GUF), integer(), fun((subject(GUH)) -> GUF)) -> GUH.
...
-spec call_forever(subject(GUJ), fun((subject(GUL)) -> GUJ)) -> GUL.
...
-spec send_after(subject(GUR), integer(), GUR) -> timer().
