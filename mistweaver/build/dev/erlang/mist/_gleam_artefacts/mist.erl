...
-opaque next(SKI, SKJ) :: {continue,
        SKI,
        gleam@option:option(gleam@erlang@process:selector(SKJ))} |
    normal_stop |
...
-opaque builder(SKK, SKL) :: {builder,
        integer(),
        fun((gleam@http@request:request(SKK)) -> gleam@http@response:response(SKL)),
...
-type websocket_message(SKM) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, SKM}.
...
-type chunk_next(SKN) :: {chunk_continue, SKN} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(SKO) -> next(SKO, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(SKS, SKT), gleam@erlang@process:selector(SKT)) -> next(SKS, SKT).
...
-spec convert_next(next(SLH, SLI)) -> mist@internal@next:next(SLH, SLI).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(SMJ)) -> gleam@http@response:response(SML))
) -> builder(SMJ, SML).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(SMP, SMQ), integer()) -> builder(SMP, SMQ).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), SMV),
    integer(),
    gleam@http@response:response(SMV)
) -> builder(mist@internal@http:connection(), SMV).
...
-spec bind(builder(SNH, SNI), binary()) -> builder(SNH, SNI).
...
-spec with_ipv6(builder(SNN, SNO)) -> builder(SNN, SNO).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(SNT, SNU), binary(), binary()) -> builder(SNT, SNU).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(SOJ)
) -> {ok, websocket_message(SOJ)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((SOP, websocket_message(SOQ), mist@internal@websocket:websocket_connection()) -> next(SOP, SOQ)),
    fun((mist@internal@websocket:websocket_connection()) -> {SOP,
        gleam@option:option(gleam@erlang@process:selector(SOQ))}),
    fun((SOP) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(SPE)) -> SPG),
    fun((SPG, SPE, s_s_e_connection()) -> gleam@otp@actor:next(SPG, SPE))
) -> gleam@http@response:response(response_data()).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(SPP)) -> SPR),
    fun((SPR, SPP, mist@internal@http:connection()) -> chunk_next(SPR))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(SPW) -> chunk_next(SPW).
