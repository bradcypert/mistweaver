...
-type valid_message(RWI) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, RWI}.

-type websocket_message(RWJ) :: {valid, valid_message(RWJ)} | invalid.
...
-type handler_message(RWK) :: {internal, gramps@websocket:frame()} | {user, RWK}.

-type websocket_state(RWL) :: {websocket_state,
        bitstring(),
        RWL,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(RXP))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(RXP))).
...
-spec apply_frames(
    ...
    fun((RXF, handler_message(RXG), websocket_connection()) -> mist@internal@next:next(RXF, RXG)),
    websocket_connection(),
    mist@internal@next:next(RXF, websocket_message(RXG)),
    fun((RXF) -> nil)
) -> mist@internal@next:next(RXF, websocket_message(RXG)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {RWU,
        gleam@option:option(gleam@erlang@process:selector(RWV))}),
    fun((RWU) -> nil),
    fun((RWU, handler_message(RWV), websocket_connection()) -> mist@internal@next:next(RWU, RWV)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
