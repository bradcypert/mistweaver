...
-spec map_response_body(
    fun((IAA) -> gleam@http@response:response(IAB)),
    fun((IAB) -> IAD)
) -> fun((IAA) -> gleam@http@response:response(IAD)).
...
-spec prepend_response_header(
    fun((IAF) -> gleam@http@response:response(IAG)),
    binary(),
    binary()
) -> fun((IAF) -> gleam@http@response:response(IAG)).
...
-spec ensure_post(gleam@http@request:request(IAJ)) -> {ok,
        gleam@http@request:request(IAJ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(IAS)) -> IAU)) -> fun((gleam@http@request:request(IAS)) -> IAU).
