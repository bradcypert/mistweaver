...
-spec set_header(request(HMK), binary(), binary()) -> request(HMK).
...
-spec prepend_header(request(HMN), binary(), binary()) -> request(HMN).
...
-spec set_body(request(any()), HMS) -> request(HMS).
set_body(Req, Body) ->
    {request,
...
-spec map(request(HMU), fun((HMU) -> HMW)) -> request(HMW).
...
-spec set_query(request(HNG), list({binary(), binary()})) -> request(HNG).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(HNK), gleam@http:method()) -> request(HNK).
...
-spec set_scheme(request(HNR), gleam@http:scheme()) -> request(HNR).
...
-spec set_host(request(HNU), binary()) -> request(HNU).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(HNX), integer()) -> request(HNX).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(HOA), binary()) -> request(HOA).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(HOD), binary(), binary()) -> request(HOD).
...
-spec remove_cookie(request(HOJ), binary()) -> request(HOJ).
