...
-type response(HWE) :: {response, integer(), list({binary(), binary()}), HWE}.

-file("src/gleam/http/response.gleam", 87).
?DOC(" Set the body of the response, overwriting any existing body.\n").
-spec set_body(response(any()), HXB) -> response(HXB).
...
-spec try_map(response(HWF), fun((HWF) -> {ok, HWH} | {error, HWI})) -> {ok,
        response(HWH)} |
    {error, HWI}.
...
-spec set_header(response(HWT), binary(), binary()) -> response(HWT).
...
-spec prepend_header(response(HWW), binary(), binary()) -> response(HWW).
...
-spec map(response(HXD), fun((HXD) -> HXF)) -> response(HXF).
...
-spec expire_cookie(response(HXO), binary(), gleam@http@cookie:attributes()) -> response(HXO).
