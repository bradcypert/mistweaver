...
-spec create(
    ...
    fun((FCS) -> FCS),
    fun((FCR, FCS) -> FCR),
    fun((FCR, FCR) -> gleam@order:order())
) -> fun((FCR, FCR, FCS) -> {ok, gleam@yielder:yielder(FCR)} | {error, nil}).
...
-spec create_infinite(
    fun((FCW) -> boolean()),
    fun((FCW, FCX) -> FCW),
    fun((FCW, FCW) -> gleam@order:order())
) -> fun((FCW, FCX) -> {ok, gleam@yielder:yielder(FCW)} | {error, nil}).
