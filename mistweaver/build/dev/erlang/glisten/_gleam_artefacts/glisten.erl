...
-type message(LCL) :: {packet, bitstring()} | {user, LCL}.
...
-type connection(LCM) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(LCM))}.

-opaque next(LCN, LCO) :: {continue,
        LCN,
        gleam@option:option(gleam@erlang@process:selector(LCO))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(LCP, LCQ) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(LCQ)) -> {LCP,
            gleam@option:option(gleam@erlang@process:selector(LCQ))}),
        fun((LCP, message(LCQ), connection(LCQ)) -> next(LCP, message(LCQ))),
        gleam@option:option(fun((LCP) -> nil)),
        integer(),
        boolean(),
        boolean(),
        ...
        gleam@option:option(gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(LCQ))))),
...
-spec continue(LDQ) -> next(LDQ, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 152).
-spec with_selector(next(LDU, LDV), gleam@erlang@process:selector(LDV)) -> next(LDU, LDV).
...
?DOC(false).
-spec convert_next(next(LEJ, LEK)) -> glisten@internal@handler:next(LEJ, LEK).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(LEP, LEQ), fun((LEQ) -> LET)) -> next(LEP, LET).
...
-spec map_user_selector(gleam@erlang@process:selector(message(LEW))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(LEW)).
...
-spec convert_loop(
    fun((LFB, message(LFC), connection(LFC)) -> next(LFB, message(LFC)))
) -> fun((LFB, glisten@internal@handler:loop_message(LFC), glisten@internal@handler:connection(LFC)) -> glisten@internal@handler:next(LFB, glisten@internal@handler:loop_message(LFC))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(LFH)) -> {LFJ,
        gleam@option:option(gleam@erlang@process:selector(LFH))})
) -> fun((glisten@internal@handler:connection(LFH)) -> {LFJ,
    gleam@option:option(gleam@erlang@process:selector(LFH))}).
...
-spec new(
    fun((connection(LFP)) -> {LFR,
        gleam@option:option(gleam@erlang@process:selector(LFP))}),
    fun((LFR, message(LFP), connection(LFP)) -> next(LFR, message(LFP)))
) -> builder(LFR, LFP).
new(On_init, Loop) ->
    {builder,
        loopback,
...
-spec with_close(builder(LFY, LFZ), fun((LFY) -> nil)) -> builder(LFY, LFZ).
...
-spec with_pool_size(builder(LGE, LGF), integer()) -> builder(LGE, LGF).
...
?DOC(false).
-spec with_http2(builder(LGK, LGL)) -> builder(LGK, LGL).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec bind(builder(LGQ, LGR), binary()) -> builder(LGQ, LGR).
...
-spec with_ipv6(builder(LGW, LGX)) -> builder(LGW, LGX).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(LHC, LHD), binary(), binary()) -> builder(LHC, LHD).
...
-spec with_connection_factory_name(
    builder(LHV, LHW),
    gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(LHW))))
) -> builder(LHV, LHW).
