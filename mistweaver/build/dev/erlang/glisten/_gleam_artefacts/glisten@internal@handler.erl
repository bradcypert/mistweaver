...
-type message(KEX) :: {internal, internal_message()} | {user, KEX}.

-type loop_message(KEY) :: {packet, bitstring()} | {custom, KEY}.

-type loop_state(KEZ, KFA) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(KFA)),
        glisten@transport:transport(),
        KEZ}.

-type connection(KFB) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(KFB))}.

-type next(KFC, KFD) :: {continue,
        KFC,
        gleam@option:option(gleam@erlang@process:selector(KFD))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(KFE, KFF) :: {handler,
        glisten@socket:socket(),
        fun((KFE, loop_message(KFF), connection(KFF)) -> next(KFE, loop_message(KFF))),
        fun((connection(KFF)) -> {KFE,
            gleam@option:option(gleam@erlang@process:selector(KFF))}),
        gleam@option:option(fun((KFE) -> nil)),
        glisten@transport:transport(),
        glisten@socket@options:active_state()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(KFS) -> next(KFS, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(KFW, KFX), gleam@erlang@process:selector(KFX)) -> next(KFW, KFX).
...
?DOC(false).
-spec start(handler(any(), KGM)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(KGM)))} |
