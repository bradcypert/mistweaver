...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(AFK)) -> {ok, AFK} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(AFE)) -> {ok, AFE} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(AFA)) -> {ok, AFA} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(AFQ), fun((AFQ) -> json())) -> json().
...
-spec array(list(AFU), fun((AFU) -> json())) -> json().
