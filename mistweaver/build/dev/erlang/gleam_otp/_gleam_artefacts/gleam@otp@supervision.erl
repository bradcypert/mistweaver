...
-type child_specification(IWH) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(IWH)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(IWI)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(IWI).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(IWN)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(IWN).
supervisor(Start) ->
...
-spec significant(child_specification(IWS), boolean()) -> child_specification(IWS).
...
-spec timeout(child_specification(IWV), integer()) -> child_specification(IWV).
timeout(Child, Ms) ->
...
-spec restart(child_specification(IWY), restart()) -> child_specification(IWY).
...
-spec map_data(child_specification(IXB), fun((IXB) -> IXD)) -> child_specification(IXD).
