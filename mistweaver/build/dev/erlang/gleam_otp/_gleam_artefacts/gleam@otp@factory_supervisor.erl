...
-opaque supervisor(IYV, IYW) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(IYV, IYW))}.

-type message(IYX, IYY) :: any() | {gleam_phantom, IYX, IYY}.

-opaque builder(IYZ, IZA) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((IYZ) -> {ok, gleam@otp@actor:started(IZA)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(IYZ, IZA)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(IZB, IZC) :: {local,
        gleam@erlang@process:name(message(IZB, IZC))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(IZD) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, IZD}.
...
-type erlang_child_spec_property(IZE, IZF) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((IZE) -> {ok, gleam@otp@actor:started(IZF)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(IZG, IZH))) -> supervisor(IZG, IZH).
get_by_name(Name) ->
...
-spec worker_child(
    fun((IZN) -> {ok, gleam@otp@actor:started(IZO)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(IZN, IZO).
...
-spec supervisor_child(
    fun((IZS) -> {ok, gleam@otp@actor:started(IZT)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(IZS, IZT).
...
-spec named(builder(IZX, IZY), gleam@erlang@process:name(message(IZX, IZY))) -> builder(IZX, IZY).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(JAG, JAH), integer(), integer()) -> builder(JAG, JAH).
...
-spec timeout(builder(JAM, JAN), integer()) -> builder(JAM, JAN).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(JAS, JAT), gleam@otp@supervision:restart()) -> builder(JAS, JAT).
...
-spec start(builder(JAY, JAZ)) -> {ok,
        gleam@otp@actor:started(supervisor(JAY, JAZ))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(JBX, JBY)) -> gleam@otp@supervision:child_specification(supervisor(JBX, JBY)).
supervised(Builder) ->
...
-spec start_child(supervisor(JCE, JCF), JCE) -> {ok,
        gleam@otp@actor:started(JCF)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((JDC) -> {ok, gleam@otp@actor:started(JDD)} |
        {error, gleam@otp@actor:start_error()}),
    JDC
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), JDD, gleam@otp@actor:start_error()).
