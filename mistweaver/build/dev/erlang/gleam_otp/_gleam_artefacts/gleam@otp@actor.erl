...
-type message(IDC) :: {message, IDC} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(IDD, IDE) :: {continue,
        IDD,
        gleam@option:option(gleam@erlang@process:selector(IDE))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(IDF, IDG) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        IDF,
        gleam@erlang@process:selector(message(IDG)),
        gleam@otp@system:debug_state(),
        fun((IDF, IDG) -> next(IDF, IDG))}.

-type started(IDH) :: {started, gleam@erlang@process:pid_(), IDH}.

-opaque initialised(IDI, IDJ, IDK) :: {initialised,
        IDI,
        gleam@option:option(gleam@erlang@process:selector(IDJ)),
        IDK}.

-opaque builder(IDL, IDM, IDN) :: {builder,
        fun((gleam@erlang@process:subject(IDM)) -> {ok,
                initialised(IDL, IDM, IDN)} |
            {error, binary()}),
        integer(),
        fun((IDL, IDM) -> next(IDL, IDM)),
        gleam@option:option(gleam@erlang@process:name(IDM))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(IDO) :: {ack, {ok, IDO} | {error, binary()}} |
...
-spec continue(IDT) -> next(IDT, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(IEF, IEG), gleam@erlang@process:selector(IEG)) -> next(IEF, IEG).
...
-spec initialised(IEM) -> initialised(IEM, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(IER, any(), IET),
    gleam@erlang@process:selector(IEX)
) -> initialised(IER, IEX, IET).
...
-spec returning(initialised(IFC, IFD, any()), IFI) -> initialised(IFC, IFD, IFI).
...
-spec new(IFM) -> builder(IFM, IFN, gleam@erlang@process:subject(IFN)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(IFS)) -> {ok, initialised(IFU, IFS, IFV)} |
        {error, binary()})
) -> builder(IFU, IFS, IFV).
...
-spec on_message(builder(IGE, IGF, IGG), fun((IGE, IGF) -> next(IGE, IGF))) -> builder(IGE, IGF, IGG).
...
-spec named(builder(IGP, IGQ, IGR), gleam@erlang@process:name(IGQ)) -> builder(IGP, IGQ, IGR).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(IHE))) -> gleam@erlang@process:selector(message(IHE)).
...
-spec receive_message(self(any(), IHA)) -> message(IHA).
...
-spec initialise_actor(
    builder(any(), any(), IHX),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, IHX} | {error, binary()})
...
-spec start(builder(any(), any(), IIK)) -> {ok, started(IIK)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(IIR), IIR) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(IIT),
    integer(),
    fun((gleam@erlang@process:subject(IIV)) -> IIT)
) -> IIV.
